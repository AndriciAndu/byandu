		...
		// -----------------------------------------------------
			...
			(function(){
				...
				// if loading website from the [index] page
					...
					infoObj.current_articleUrl     = infoObj.navElements_article [0].getAttribute('data-url').replace(/\//g, '');
		...
		// --------------------------------------------------
			...
			// --------------------------------------------------
				...
				routeObject.navElements_article_updateActiveStatus = function() {
					...
					items.map(function(item) {
						if (item.getAttribute('data-url').toLowerCase() == articleUrl)  { item.classList   .add('active') } 
						else 											 				{ item.classList.remove('active') }
					})
				};
			...
			// --------------------------------------------------
				...
				routeObject.articleClick = function(elem) {
					...
					var articleUrl   = elem.getAttribute('data-url');
	...
	(function(){
		...
		// --------------------------------------------------
			...
			articleButtons.map(function(item) { 
				item.addEventListener('click', function() { routeObject.articleClick(this) })
			});
	})();
