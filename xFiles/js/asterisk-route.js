...
// --------------------------------------------------
	...
	// ***REQUIRED*** to be set both [here] and in [asterisk-route.js]
		...
		routeObject.whenNewArticle_updateItems = [
			{
				identifier : '.__route__articleBtn' , 
				targetProperty : 'href'
			} , 

			{
				identifier : '.__route__articleGroup' , 
				targetProperty : 'data-categoryName'
			}
		];

		routeObject.templates = [
			{ 
				urlIdentifier : '/utility/gridSystem-12col' , 
				filePath_html : '/utility/gridSystem-12col.html' , 
				filePath_js   : '../../js/extra_scripts/utility/gridSystem-12col.js' , 
				articleParameters : {
					category : 'utility' ,
					article  : 'gridSystem-12col.html'
				} 
			}
		];

		// elements outside the __route__mainView - which must be updated depending on the current article

		function updateItems_whenNewPageIsLoaded(articleParams__obj) {

			var myTargets_1 = document.getElementsByClassName('__route__category');
			var identifierValue = articleParams__obj.category;
			...
			var myTargets_2 = document.getElementsByClassName('__route__articleBtn');
			var identifierValue = articleParams__obj.article;

			myTargets_2.map(function(elem){
				if (elem.href == identifierValue) { 
					elem.classList.add   ('active') 
				} else { 
					elem.classList.remove('active') 
				}
		...
		// --------------------------------------------------
			...
			// --------------------------------------------------
				...
				routeObject.fetchAndUpdateInnerHTML = function(urlString) { 
					...
					.then(function(text) { 
						console.log('before remveEvents');
						console.log(articleFunctions);
						console.log(articleEvents);
						console.log('---');
						article_unload();
						console.log('after remveEvents');
						console.log(articleFunctions);
						console.log(articleEvents);
						console.log('---');
